...
async function testMetricas() {
  ...
  try {
    ...
    const { runFCFSSandbox } = await import('../../src/lib/engine/engine');
    const trace = runFCFSSandbox(procesos, { TFP: 1 });
    
    const { MetricsBuilder } = await import('../../src/lib/metrics/metricas');
...
async function testGantt() {
  ...
  try {
    ...
    const { GanttBuilder } = await import('../../src/lib/gantt/builder');
